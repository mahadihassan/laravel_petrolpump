                            ...
                            <table class="table table-striped table-bordered table-hover" id="sample_1">
                                ...
                                @foreach($repayments as $k => $repayment)
                                    <tr>
                                        ...
                                        <td>{{$basic->symbol}} {{$repayment->old_balance}}</td>
                                        <td>{{$basic->symbol}} {{$repayment->payment}}</td>
                                        <td>{{$basic->symbol}} {{$repayment->new_balance}}</td>
                                        <td>{{$repayment->note}}</td>
                                        <td>
                                            <button type="button" class="btn btn-sm btn-danger bold uppercase delete_button"
                                                    data-toggle="modal" data-target="#DelModal"
                                                    data-id="{{ $repayment->id }}" title="Delete">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content" >
                ...
                <div class="modal-footer">
                    <form method="post" action="{{ route('repayment-delete') }}" class="form-inline">
                        {!! csrf_field() !!}
                        {{ method_field('DELETE') }}
