...
use App\TraitsFolder\HttpsTrait;
use App\TraitsFolder\MaintenanceTrait;
...
class BasicSettingController extends Controller
{
    ...
    protected function putBasicSetting(Request $request,$id)
    {
        ...
        $in = Input::except('_method','_token','web_mood','https');
        $in['email_verify'] = $request->email_verify == 'on' ? '1' : '0';
        $in['phone_verify'] = $request->phone_verify == 'on' ? '1' : '0';
        $this->updateMaintenance($request->web_mood);
        $this->updateHttps($request->https);
        $basic->fill($in)->save();
