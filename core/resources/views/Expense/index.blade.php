            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                         <a href="{{ route('expense-create') }}" class="btn btn-primary bold"><i class="fa fa-plus"></i> Add New Expense</a>
                            ...
                            <table class="table table-striped table-bordered table-hover" id="sample_1">
                                ...
                                @foreach($expenses as $k => $expense)
                                    <tr>
                                        <td>{{ ++$k }}</td>
                                        <td>{{$expense->name}}</td>
                                        <td>{{$basic->symbol}} {{$expense->cost}}</td>
                                        <td>
                                            @if($expense->status == 1)
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content" >
                ...
                <div class="modal-footer">
                    <form method="post" action="{{ route('expense-delete') }}" class="form-inline">
                        {!! csrf_field() !!}
                        {{ method_field('DELETE') }}
